	...
	class StatisticsController extends BaseController{
		...
		public function __construct(){
			...
			$this->beforeFilter(function ($route, $request) use ($controller){
				...
				DB::table('statistic_requests')->insert(array('server' => $ip, 'requested_on' => Carbon::now(), 'route' => '/'.$route->getPath(),));
		...
		public function set(){
			...
			foreach(array_keys($keys) as $key){
				if(ListSentence::startsWith($key, 'p')){
					...
					$count = DB::table('plugin_statistics')->where('inserted_on', $time)->where('server', $server)->where('plugin', $plugin)->get();
					...
					if(count($count) == 0 && count($plugins) > 0){
						...
						DB::table('plugin_statistics')->insert(array('server' => $server, 'plugin' => $plugin, 'version' => $value, 'inserted_on' => $time));
					}
				}else{
					$count = DB::table('server_statistics')->where('inserted_on', $time)->where('server', $server)->where('key', $key)->select('key')->get();
					...
					if(count($count) == 0){
						if(!(in_array($key, $count) && in_array($key, Config::get('statistics.limited-keys')))){
							...
							if(in_array($key, $allowedKeys)){
								...
								DB::table('server_statistics')->insert(array('server' => $server, 'key' => $key, 'value' => $value, 'inserted_on' => $time));
							}else{
								$error[$key] = 'invalid';
							}
						}else{
		...
		public function get($column, $key, $server = null){
			...
			if($server == null){
				$query = DB::table('server_statistics')->where('key', $key)->select($column)->get();
			}else{
				$query = DB::table('server_statistics')->where('server', $server)->where('key', $key)->select($column)->get();
			}
		...
		public function getTotalServers(){
			$table = DB::table('server_statistics')->where('key', 'bPlayersOnline')->select(DB::raw('inserted_on as timestamp'), DB::raw('count(*) as servers'), DB::raw('sum(value) as players'))->groupBy('inserted_on')->get();
